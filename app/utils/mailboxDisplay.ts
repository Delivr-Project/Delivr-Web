...
export class MailboxDisplayUtils {

    public static isInbox(mb: Mailbox): boolean {
     ...
     */
    public static findMailboxByUrlSegments(mailboxes: Mailbox[], segments: string[]): Mailbox | undefined {
        ...
        if (segments.length === 1 && segments[0]!.toLowerCase() === 'inbox') {
            const inbox = mailboxes.find(this.isInbox);
     ...
     */
    public static crumbs(mb: Mailbox, mailboxes: Mailbox[], accountId: number): MailboxDisplayUtils.Crumb[] {
        const parts = mb.path.split(mb.delimiter);
        const items: MailboxDisplayUtils.Crumb[] = [];
        let accum = '';
    ...
    public static rollupUnseen(node: MailboxTreeNode): number {
        let total = node.mailbox?.status.unseen ?? 0;
        for (const child of node.children) total += this.rollupUnseen(child);
...
export namespace MailboxDisplayUtils {
